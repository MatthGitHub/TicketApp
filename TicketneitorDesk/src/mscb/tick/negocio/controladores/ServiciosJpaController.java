    ...
    public void create(Servicios servicios) {
        ...
        try {
            ...
            for (Tickets ticketsListTickets : servicios.getTicketsList()) {
                Servicios oldServicioOfTicketsListTickets = ticketsListTickets.getServicio();
                ticketsListTickets.setServicio(servicios);
                ticketsListTickets = em.merge(ticketsListTickets);
                if (oldServicioOfTicketsListTickets != null) {
                    oldServicioOfTicketsListTickets.getTicketsList().remove(ticketsListTickets);
                    oldServicioOfTicketsListTickets = em.merge(oldServicioOfTicketsListTickets);
                }
            }
        ...
        try {
            ...
            for (Tickets ticketsListOldTickets : ticketsListOld) {
                if (!ticketsListNew.contains(ticketsListOldTickets)) {
                    ...
                    illegalOrphanMessages.add("You must retain Tickets " + ticketsListOldTickets + " since its servicio field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Tickets ticketsListNewTickets : ticketsListNew) {
                if (!ticketsListOld.contains(ticketsListNewTickets)) {
                    Servicios oldServicioOfTicketsListNewTickets = ticketsListNewTickets.getServicio();
                    ticketsListNewTickets.setServicio(servicios);
                    ticketsListNewTickets = em.merge(ticketsListNewTickets);
                    if (oldServicioOfTicketsListNewTickets != null && !oldServicioOfTicketsListNewTickets.equals(servicios)) {
                        oldServicioOfTicketsListNewTickets.getTicketsList().remove(ticketsListNewTickets);
                        oldServicioOfTicketsListNewTickets = em.merge(oldServicioOfTicketsListNewTickets);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Tickets ticketsListOrphanCheckTickets : ticketsListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Servicios (" + servicios + ") cannot be destroyed since the Tickets " + ticketsListOrphanCheckTickets + " in its ticketsList field has a non-nullable servicio field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
